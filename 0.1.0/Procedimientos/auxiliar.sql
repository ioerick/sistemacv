...
insert into auxiliar values (pk, c1, c2, c3, 1);
  insert into historial_actividades (accion, usuario, tabla, campo) values ('add', usuario, 'auxiliar', pk);
end;
$body$
language plpgsql;
...
update auxiliar set codpersona = c1, codcontrato = c2, codespecialidad = c3  where codauxiliar = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('edit', usuario, 'auxiliar', pk);
end;
$body$
language plpgsql;
...
update auxiliar set codestado = 0 where codauxiliar = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('delete', usuario, 'auxiliar', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('search', usuario, 'auxiliar', pk);
end;
$body$
language plpgsql;
