...
insert into correo values (pk, c1, 1);
  insert into historial_actividades (accion, usuario, tabla, campo) values ('add', usuario, 'correo', pk);
end;
$body$
language plpgsql;
...
update correo set descripcion = c1 where codcorreo = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('edit', usuario, 'correo', pk);
end;
$body$
language plpgsql;
...
update correo set codestado = 0 where codcorreo = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('delete', usuario, 'correo', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('search', usuario, 'correo', pk);
end;
$body$
language plpgsql;
