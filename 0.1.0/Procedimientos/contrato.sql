...
insert into contrato values (pk, c1, c2, c3, 1);
  insert into historial_actividades (accion, usuario, tabla, campo) values ('add', user, 'contrato', pk);
end;
$body$
language plpgsql;
...
update contrato set descripcion = c1, fechainicio = c2, fechafinal = c3 where codcontrato = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('edit', user, 'contrato', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('delete', user, 'contrato', pk);
  end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('search', user, 'contrato', pk);
end;
$body$
language plpgsql;
