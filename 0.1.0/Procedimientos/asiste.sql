...
insert into asist values (pk, c1, c2, c3, c4, c5, c6, 1);
  insert into historial_actividades (accion, usuario, tabla, campo) values ('add', user, 'asiste', pk);
end;
$body$
language plpgsql;
...
update asiste set codanimal = c1, codservicio = c2, codveterinario = c3, codauxiliar = c4, fecha = c5, descripcion = c6 where codasiste = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('edit', user, 'asiste', pk);
end;
$body$
language plpgsql;
...
update asiste set codestado = 0 where codasiste = pk and codestado != 0;
  insert into historial_actividades (accion, usuario, tabla, campo) values ('delete', user, 'asiste', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('search', user, 'asiste', pk);
end;
$body$
language plpgsql;
