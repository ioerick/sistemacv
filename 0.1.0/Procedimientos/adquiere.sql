...
insert into adquiere values( pk, c1, c2, c3, c4, c5, 1);
	insert into historial_actividades (accion, usuario, tabla, campo) values ('add', usuario, 'adquiere', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('edit', usuario, 'adquiere', pk);
end;
$body$
language plpgsql;
...
update adquiere set codestado = 0 where codadquiere = pk and codestado !=0;
	insert into historial_actividades (accion, usuario, tabla, campo) values ('delete', usuario, 'adquiere', pk);
end;
$body$
language plpgsql;
...
insert into historial_actividades (accion, usuario, tabla, campo) values ('search', usuario, 'adquiere', pk);
end;
$body$
language plpgsql;
